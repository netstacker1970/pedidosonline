...
import br.pedidosonline.domain.Produto;
import br.pedidosonline.repository.ProdutoRepository;
...
/**
 * REST controller for managing {@link br.pedidosonline.domain.Produto}.
 */
@RestController
@RequestMapping("/api")
@Transactional
        ...
        Optional<Produto> result = produtoRepository
            ...
            .map(existingProduto -> {
                if (produto.getProduto() != null) {
                    existingProduto.setProduto(produto.getProduto());
                }
                if (produto.getImage() != null) {
                    existingProduto.setImage(produto.getImage());
                }
                if (produto.getImageContentType() != null) {
                    existingProduto.setImageContentType(produto.getImageContentType());
                }
